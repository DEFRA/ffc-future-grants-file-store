...
const { saveMetadataHandler, deleteMetadataHandler, initUserDataReceiver } = require('./utils/recieveQueueHelperFunctions');
const fs = require('fs');
...
async function initializeMessageReceivers() {
    userDataReceiver = new MessageReceiver(userDataRequestQueueAddress, async (msg) => {
...
async function runSqlScript() {
    ...
    try {
        if (process.env.NODE_ENV === 'production') {
            ...
            await client.query(`SET SESSION AUTHORIZATION DEFAULT, PUBLIC, ${accessToken.token}`);
        }
